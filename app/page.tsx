...
function PageHero({ eyebrow, title, body, image, images, children, status }: { eyebrow: string; title: string; body: string; image?: string; images?: string[]; children?: ReactNode; status?: string }) {
  return <section className="page-hero">
    <div className="shell page-hero-grid">
      ...
      {images?.length ? <div className="page-hero-image page-hero-collage" aria-hidden="true">{images.map((src) => <img key={src} src={assetPath(src)} alt="" />)}</div> : image && <div className="page-hero-image"><img src={assetPath(image)} alt="" /></div>}
    </div>
  </section>;
}
...
function Comunidad({ go }: { go: (page: PageKey) => void }) {
  ...
  return <>
    <PageHero eyebrow="La comunidad" title="Una comunidad para quienes ya han empezado a construir." body="CLH reúne a jóvenes de España y Latinoamérica que lideran proyectos, asociaciones e iniciativas desde etapas tempranas. Personas con visión, criterio, valores y capacidad para convertir una idea en movimiento." images={["/images/members/michelle-rogel.webp", "/images/members/santiago-narino-rivera.webp", "/images/members/camila-calvo.webp", "/images/members/sophia-de-leone.webp"]}><Button variant="light" onClick={() => go("miembros")}>Conoce a nuestros miembros</Button></PageHero>
...
function Capitales({ go }: { go: (page: PageKey) => void }) {
  ...
  return <>
    <PageHero eyebrow="Los 4 pilares de la comunidad" title="Una forma completa de desarrollar criterio, relaciones, experiencia y capacidad de generar valor." body="CLH se construye sobre cuatro pilares conectados. Cada uno trabaja una dimensión distinta del liderazgo y todos se refuerzan entre sí." images={["/images/members/jorge-gregorio-garcia-heras.jpg", "/images/members/renata-berho.webp", "/images/members/karen-jimena-fonseca.webp", "/images/members/laura-nayeli-mendez-martinez.webp"]} />
...
function Miembros({ go }: { go: (page: PageKey) => void }) {
  ...
  return <>
    <PageHero eyebrow="Nuestros miembros" title="El valor de CLH está en las personas que deciden construir juntas." body="Más de 100 líderes jóvenes de España y Latinoamérica forman una red conectada por la iniciativa, el criterio y la voluntad de aportar." images={["/images/members/raphael-andersor-sanchez-pinto.webp", "/images/members/krissia-alejandra-bueno.webp", "/images/members/emily-torres-pineda.webp", "/images/members/rodrigo-lopez-bolanos.webp"]} />
    <div className="stat-band-frame"><div className="shell"><StatBand /></div></div>
...
function Beway({ go }: { go: (page: PageKey) => void }) {
  ...
  return <div className="beway-page">
    <section className="beway-hero"><div className="shell beway-hero-grid"><div className="beway-hero-copy"><span className="beway-status">Lanzamiento · Julio de 2027</span><p className="beway-kicker">Ecosistema de talento, innovación y oportunidades</p><h1>Empresas y universitarios conectados por lo que pueden construir juntos.</h1><p>Una plataforma impulsada desde CLH para activar proyectos, entender a la nueva generación y descubrir talento mediante evidencias reales de ejecución.</p><div className="button-row"><Button variant="light" onClick={() => go("organizaciones")}>Soy una empresa</Button><Button variant="ghost" onClick={() => document.getElementById("beway-modelo")?.scrollIntoView({behavior:"smooth"})}>Conocer el proyecto</Button></div></div><div className="beway-brand-lockup" aria-label="Vista conceptual de la plataforma Beway"><div className="beway-lockup-header"><strong>BEWAY</strong><span>Vista conceptual</span></div><div className="beway-lockup-card"><small>RETO INTERNACIONAL</small><h2>El talento se descubre cuando entra en acción.</h2><p>Equipos universitarios, empresas y evidencias reales de ejecución.</p><div className="beway-lockup-footer"><div className="beway-lockup-members" aria-hidden="true"><img src={assetPath("/images/members/jorge-gregorio-garcia-heras.jpg")} alt="" /><img src={assetPath("/images/members/camila-calvo.webp")} alt="" /><img src={assetPath("/images/members/michelle-rogel.webp")} alt="" /></div><span>Conectar · Construir · Demostrar</span></div></div></div></div><div className="shell beway-principles">{principles.map((principle, i) => <div key={principle}><span>{String(i + 1).padStart(2, "0")}</span><strong>{principle}</strong></div>)}</div></section>
...
function Semana({ go }: { go: (page: PageKey) => void }) {
  ...
  return <>
    <PageHero status="Madrid · Julio de 2027" eyebrow="Cumbre CLH × Beway" title="Donde la comunidad, el talento y las grandes empresas miran hacia el mismo futuro." body="Un encuentro internacional con directivos de grandes empresas, conversaciones sobre liderazgo, innovación y futuro del trabajo, y la presentación oficial de la plataforma Beway." images={["/images/members/jorge-gregorio-garcia-heras.jpg", "/images/members/camila-calvo.webp", "/images/members/michelle-rogel.webp", "/images/members/santiago-narino-rivera.webp"]}><Button variant="light" onClick={() => go("contacto")}>Solicitar el dossier</Button><Button variant="ghost" onClick={() => go("organizaciones")}>Participar en la Cumbre</Button></PageHero>
...
function Contacto() {
  ...
  return <>
    <PageHero eyebrow="Contacto" title="Dinos qué quieres construir con CLH." body="Elige el motivo de tu consulta y comparte el contexto necesario. Así podremos dirigirla desde el principio a la persona adecuada." images={["/images/members/michelle-rogel.webp", "/images/members/camila-calvo.webp", "/images/members/santiago-narino-rivera.webp", "/images/members/karen-jimena-fonseca.webp"]} />
